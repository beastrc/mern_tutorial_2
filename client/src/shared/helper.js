...
const openSendMessagePopup = (self, seekerId, isInvite = false, cb) => {
  ...
  const inviteYesBtnAction = function(subject, msg, jobId) {
    ...
    _sendMessage(self, popupType, sendMsgObj, cb);
  };
...
const _sendMessage = (self, popupType, sendMsgObj, cb) => {
  utils.apiCall('SEND_MESSAGE', { data: sendMsgObj }, function(err, response) {
    if (err) {
      ...
      if (utils.isResSuccess(response)) {
        utils.modalPopup(popupType, 'hide', self);
        setTimeout(() => {
          openSuccessMessagePopup(self, 'SEND_MSG_SUCCESS', cb);
        }, 600);
      } else {
