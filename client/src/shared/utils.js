...
const redirectionHandle = () => {
  if (getUserRole() === 'admin') {
    ...
    if (isSeekerProfileCompleted() || isPosterProfileCompleted()) {
      if(document.location.hasOwnProperty('previousState') && document.location.previousState.includes('/my-posted-jobs/')){
        changeUrl(document.location.previousState);
      }else{
        goToHome();
      }
      ...
      if (page) {
        page = page === constant['ROUTES_PATH']['PROJECT_SEARCH'];
      }
