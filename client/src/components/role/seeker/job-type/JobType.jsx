...
export default class JobType extends React.Component {
        ...
        if (utils.isResSuccess(response)) {
          ...
          for (var eTypesObj of data['employment_types']) {
            if (eTypesObj['name'] === 'Permanent') {
              that.state['formData']['desired_job_type'][0][
                'employment_type_id'
              ] = eTypesObj['_id'];
            } else if (eTypesObj['name'] === 'Contract') {
              that.state['formData']['desired_job_type'][1][
                'employment_type_id'
              ] = eTypesObj['_id'];
            }
          }
  ...
  componentDidMount() {
    ...
    $('.work-permanent .pmd-checkbox > input[type=checkbox]').change(
      function() {
        formVal = thisObj.state.formData;
        if ($(this).is(':checked')) {
    ...
    $('.work-part-time .pmd-checkbox > input[type=checkbox]').change(
      function() {
        formVal = thisObj.state.formData;
      }
    );
    $('.work-full-time .pmd-checkbox > input[type=checkbox]').change(
      function() {
        formVal = thisObj.state.formData;
      }
    );
      ...
      function(err, response) {
        if (err) {
          ...
          if (response.data.Code == 200 && response.data.Status == true) {
            ...
            formData.willing_to_work_location_id =
              data.willing_to_work_location_id;
            ...
            if (data.willing_to_work_part_time == 'Y') {
              ...
              formData.willing_to_work_part_time =
                data.willing_to_work_part_time;
            }
            ...
            if (data.willing_to_work_full_time == 'Y') {
              ...
              formData.willing_to_work_full_time =
                data.willing_to_work_full_time;
            }
            ...
            if (data.desired_job_type.length > 0) {
              if (data.desired_job_type[0].selected == 'Y') {
                ...
                that.multiPermanentRange(
                  data['desired_job_type'][0]['min_amount'],
                  data['desired_job_type'][0]['max_amount']
                );
              ...
              if (data.desired_job_type[1].selected == 'Y') {
                ...
                that.multiContractRange(
                  data['desired_job_type'][1]['min_amount'],
                  data['desired_job_type'][1]['max_amount']
                );
              ...
              document.getElementById('permanent-value-min').innerHTML =
                '$' + data['desired_job_type'][0]['min_amount'];
              document.getElementById('permanent-value-max').innerHTML =
                '$' + data['desired_job_type'][0]['max_amount'];
              formData['desired_job_type'][0]['min_amount'] =
                data['desired_job_type'][0]['min_amount'];
              formData['desired_job_type'][0]['max_amount'] =
                data['desired_job_type'][0]['max_amount'];
              document.getElementById('contract-value-min').innerHTML =
                '$' + data['desired_job_type'][1]['min_amount'];
              document.getElementById('contract-value-max').innerHTML =
                '$' + data['desired_job_type'][1]['max_amount'];
              formData['desired_job_type'][1]['min_amount'] =
                data['desired_job_type'][1]['min_amount'];
              formData['desired_job_type'][1]['max_amount'] =
                data['desired_job_type'][1]['max_amount'];
            } else {
  ...
  multiPermanentRange(val1, val2) {
    ...
    var pmdSliderValueRange = document.getElementById(
      'pmd-slider-permanent-rate'
    );
  ...
  handleInput(e) {
    ...
    if (
      name == 'willing_to_work_locally' ||
      name == 'willing_to_work_remotely' ||
      name == 'willing_to_work_full_time' ||
      name == 'willing_to_work_part_time'
    ) {
      if (target.value == 'N') {
      ...
      if (target.value == 'N') {
        ...
        if (name == 'work-contract') {
          formD['desired_job_type'][0]['min_amount'] = document
            .getElementById('permanent-value-min')
            .innerHTML.replace('$', '');
          formD['desired_job_type'][0]['max_amount'] = document
            .getElementById('permanent-value-max')
            .innerHTML.replace('$', '');
          formD['desired_job_type'][1]['selected'] = 'Y';
        } else if (name == 'work_permanent') {
          formD['desired_job_type'][1]['min_amount'] = document
            .getElementById('permanent-value-min')
            .innerHTML.replace('$', '');
          formD['desired_job_type'][1]['max_amount'] = document
            .getElementById('permanent-value-max')
            .innerHTML.replace('$', '');
          formD['desired_job_type'][1]['selected'] = 'Y';
        }
  ...
  handleProceed(e) {
    ...
    if (this.state.work_permanent == 'Y') {
      formData['desired_job_type'][0]['min_amount'] = document
        .getElementById('permanent-value-min')
        .innerHTML.replace('$', '');
      formData['desired_job_type'][0]['max_amount'] = document
        .getElementById('permanent-value-max')
        .innerHTML.replace('$', '');
      formData['desired_job_type'][0]['min_amount'] = formData[
        'desired_job_type'
      ][0]['min_amount'].replace(/,/g, '');
      formData['desired_job_type'][0]['max_amount'] = formData[
        'desired_job_type'
      ][0]['max_amount'].replace(/,/g, '');
    ...
    if (this.state.work_contract == 'Y') {
      formData['desired_job_type'][1]['min_amount'] = document
        .getElementById('contract-value-min')
        .innerHTML.replace('$', '');
      formData['desired_job_type'][1]['max_amount'] = document
        .getElementById('contract-value-max')
        .innerHTML.replace('$', '');
      formData['desired_job_type'][1]['min_amount'] = formData[
        'desired_job_type'
      ][1]['min_amount'].replace(/,/g, '');
      formData['desired_job_type'][1]['max_amount'] = formData[
        'desired_job_type'
      ][1]['max_amount'].replace(/,/g, '');
    ...
    this.setState({ formData }, function() {
      if (!_this.state.formErrors.willing_to_work_location_id) {
        utils.apiCall('SET_USER_JOB_PROFILE', { data: obj }, function(
          err,
          response
        ) {
          if (err) {
  ...
  logChange(val) {
    ...
    if (
      formData.willing_to_work_location_id.length <= 0 &&
      this.refs.willing_to_work_locally.checked
    ) {
      fieldValidationErrors.willing_to_work_location_id =
        constant.ENTER_LOCATION;
    } else if (
      formData.willing_to_work_location_id.length <= 0 &&
      !this.refs.willing_to_work_locally.checked
    ) {
    ...
    return (
      ...
      >
        <div className="visible-xs mobile-page-heading">
          <span
            className="previous"
            onClick={() => utils.changeUrl(routesPath['SEEKER_HEADLINE'])}
          ></span>{' '}
          Job Type
          <span
            className={
              this.state.completeStatus >= 4 ? 'next' : 'next disabled-element'
            }
            onClick={() => utils.changeUrl(routesPath['SEEKER_GET_STARTED'])}
          ></span>{' '}
        </div>
          ...
          <div className="job-card card">
            ...
            <div className="work-within checkbox pmd-default-theme row m-0">
              <label className="pmd-checkbox col-sm-3 pt-5">
                <input
                  ref="willing_to_work_locally"
                  value={this.state.formData.willing_to_work_locally}
                  name="willing_to_work_locally"
              ...
              <div
                className={
                  this.state.formErrors.willing_to_work_location_id !== ''
                    ? 'select-parent col-sm-6 global-error'
                    : 'select-parent col-sm-6'
                }
              >
                <Select
                  multi
                ...
                />
                <span className="col-sm-3 pl-0 full-width clr">
                  {this.state.formErrors.willing_to_work_location_id !== ''
                    ? this.state.formErrors.willing_to_work_location_id
                    : ''}
                </span>
            ...
            <div className="work-remotely checkbox pmd-default-theme m-0">
              <label className="pmd-checkbox">
                <input
                  ref="willing_to_work_remotely"
                  type="checkbox"
                  value={this.state.formData.willing_to_work_remotely}
                  name="willing_to_work_remotely"
                  onChange={this.handleInput.bind(this)}
                />
                <span className="pmd-checkbox-label">&nbsp;</span>
            ...
            <div className="work-part-time checkbox pmd-default-theme">
              <label className="pmd-checkbox">
                <input
                  ref="willing_to_work_part_time"
                  type="checkbox"
                  value={this.state.formData.willing_to_work_part_time}
                  name="willing_to_work_part_time"
                  onChange={this.handleInput.bind(this)}
                />
                <span className="pmd-checkbox-label">&nbsp;</span>
            ...
            <div className="work-full-time checkbox pmd-default-theme">
              <label className="pmd-checkbox">
                <input
                  ref="willing_to_work_full_time"
                  type="checkbox"
                  value={this.state.formData.willing_to_work_full_time}
                  name="willing_to_work_full_time"
                  onChange={this.handleInput.bind(this)}
                />
                <span className="pmd-checkbox-label">&nbsp;</span>
            ...
            <div className="work-permanent checkbox pmd-default-theme">
              <label className="pmd-checkbox">
                <input
                  ...
                  name="work_permanent"
                  type="checkbox"
                  value={this.state.work_permanent}
                  onChange={this.handleInput.bind(this)}
                />
                <span className="pmd-checkbox-label">&nbsp;</span>
              ...
              <span className="permanent-slidder-wrapper slider-disabled">
                ...
                <span
                  id="pmd-slider-permanent-rate"
                  className="pmd-range-slider noUi-target noUi-ltr noUi-horizontal noUi-background"
                ></span>
            ...
            <div className="work-contract checkbox pmd-default-theme">
              <label className="pmd-checkbox">
                <input
                  ...
                  name="work_contract"
                  type="checkbox"
                  value={this.state.work_contract}
                  onChange={this.handleInput.bind(this)}
                />
                <span className="pmd-checkbox-label">&nbsp;</span>
