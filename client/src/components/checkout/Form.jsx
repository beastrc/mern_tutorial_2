...
const CheckoutForm = ({planKey, planPrice, router}) => {
	const stripe = useStripe()
  	...
  	const handleSubmit = async (event) => {
	    ...
	    if (payload.error) {
	    	...
	    	utils.apiCall('CREATE_SUBSCRIPTION', {'data': { paymentMethodId, planKey, planPrice }}, function(err, response) {

		      if (err) {
						//utils.flashMsg('show', 'Error while creating subscription');
						utils.flashMsg('show', err);
		        ...
		        if(response.status === 200){
							...
							router.push(constant['ROUTES_PATH']['SUBSCRIPTIONS'])
						}
