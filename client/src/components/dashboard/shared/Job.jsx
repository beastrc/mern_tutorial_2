...
export default class Job extends React.Component {
  ...
  updatePostedJobStatus(jobId, action) {
    ...
    req.job_id = jobId;
    req.status = action;
    utils.apiCall('UPDATE_POSTED_JOB_STATUS', { data: req }, function (
      err,
      response
    ) {
      if (err) {
  ...
  render() {
    ...
    if (job.fromRoute === 'POSTED_JOBS') {
      ...
      switch (posterHighestStep) {
        case constant['JOB_STEPS']['INTERVIEWING']:
          postedJobStatusText = 'Interviewing';
          break;
        case constant['JOB_STEPS']['N_TERMS']:
          postedJobStatusText = 'Negotiating Terms';
          break;
        case constant['JOB_STEPS']['S_PENDING']:
          postedJobStatusText = 'Start Pending';
          break;
        case constant['JOB_STEPS']['IN_PROGRESS']:
          postedJobStatusText = 'In Progress';
          break;
        case constant['JOB_STEPS']['J_COMPLETE']:
          postedJobStatusText = 'Complete';
          break;
      }
      // }
    } else {
      ...
      if (this.state.jobStatus < 0) {
        ...
        switch (this.state.jobStatus) {
          case constant['JOB_STEPS']['APPLIED']:
            appliedJobStatusText = 'Applied';
            break;
          case constant['JOB_STEPS']['INTERVIEWING']:
            appliedJobStatusText = 'Interviewing';
            break;
          case constant['JOB_STEPS']['N_TERMS']:
            appliedJobStatusText =
              job.nTermStatus === constant['N_TERMS_STATUS']['SENT']
                ? 'Negotiating Terms'
                : 'Interviewing';
            break;
          case constant['JOB_STEPS']['S_PENDING']:
            appliedJobStatusText = 'Start Pending';
            break;
          case constant['JOB_STEPS']['IN_PROGRESS']:
            appliedJobStatusText = 'In Progress';
            break;
          case constant['JOB_STEPS']['J_COMPLETE']:
            appliedJobStatusText = 'Complete';
            break;
        }
      }
    }
    ...
    if (
      ...
      this.state.isBarIdValid.toLowerCase() === 'no'
    ) {
    ...
    var jobBtnClass = classNames({
      ...
      'btn-applied':
        (this.state.jobStatus > constant['JOB_STEPS']['APPLY'] &&
          this.state.jobStatus < constant['JOB_STEPS']['S_PENDING']) ||
        (job.fromRoute === 'POSTED_JOBS' &&
          this.state.posterJobStatus === constant['STATUS']['ACTIVE'] &&
          posterHighestStep < constant['JOB_STEPS']['S_PENDING']),
      'btn-grey-disabled':
        this.state.jobStatus < 0 && job.fromRoute !== 'POSTED_JOBS',
      'btn-unposted':
        job.fromRoute === 'POSTED_JOBS' &&
        this.state.posterJobStatus === constant['STATUS']['INACTIVE'],
      'btn-saved':
        (this.state.jobStatus > constant['JOB_STEPS']['N_TERMS'] &&
          this.state.jobStatus < constant['JOB_STEPS']['J_COMPLETE']) ||
        (posterHighestStep > constant['JOB_STEPS']['N_TERMS'] &&
          posterHighestStep < constant['JOB_STEPS']['J_COMPLETE']),
      'btn-done':
        this.state.jobStatus === constant['JOB_STEPS']['J_COMPLETE'] ||
        posterHighestStep === constant['JOB_STEPS']['J_COMPLETE'],
          ...
          <div className="hourly-estimate col-xs-3 col-sm-2 p-0">
            {job.negotiateLaterStatus ? (
              <span style={{ fontSize: '20px', textTransform: 'capitalize' }}>
                $ Rate Negotiable
                <div>
                  {job.rateType ? job.rateType.toLowerCase() : job.rateType}
                </div>
              </span>
            ) : (
                <span>
          ...
          {practiceAreas.length > 0 ? (
            <div className="col-sm-4">
              ...
              {practiceAreas.length > 2 ? (
                <span className="d-inline-block v-bottom">
                  {' '}
                  +{practiceAreas.length - 2}
                </span>
              ) : (
        ...
        <div className="card-content-footer row">
          <div className="col-sm-4">
            <div className="pb-5">
              Posted:
              <span>
                {this.state.posterJobStatus && this.state.postedAt
                  ? utils
                    .convertUtcToEst(this.state.postedAt)
                    .format(constant['JOB_DATE_FORMAT'])
                  : constant['NO_DATA_SYMBOL']}
              </span>
            </div>
            <div className="pb-5">
              State:
              <span>{job.state_info.name}</span>
            </div>
            <div>
              Desired Start Date:
              <span>
                {job.estimatedStartDate
                  ? moment(job.estimatedStartDate).format(
                    constant['JOB_DATE_FORMAT']
                  )
                  : 'ASAP'}
              </span>
            </div>
          ...
          <div className="col-sm-8 text-right">
            ...
            */}
            ...
            {job.fromRoute === 'POSTED_JOBS' ? (
              <button
                type="button"
                className={jobBtnClass}
                disabled={
                  this.state.posterJobStatus === constant['STATUS']['ACTIVE']
                }
                onClick={this.updatePostedJobStatus.bind(
                  this,
                  job._id,
              ...
              >
                ...
                <button
                  ...
                  className={jobBtnClass}
                  disabled={
                    this.state.jobStatus !== constant['JOB_STEPS']['APPLY'] || this.props.PremiumSeekerPost == 0
                  }
                  onClick={this.updateJobStatus.bind(
                    this,
                    job._id,
