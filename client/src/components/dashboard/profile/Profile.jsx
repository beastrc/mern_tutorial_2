...
import { constant, utils, config, helper, cookieManager } from '../../../shared/index';
...
export default class Profile extends React.Component {
  constructor(props) {
    ...
    this.handleAvailSelect = this.handleAvailSelect.bind(this);
  }
      ...
      function(err, response) {
        if (err) {
          ...
          if (response.data.Code == 200 && response.data.Status == true) {
            ...
            that.setState({userId: responseData._id})
  ...
  handleAvailSelect(eventKey, event) {
    var _this = this;
    utils.apiCall('SET_AVAILABILITY_PROFILE', { 'data': {availability_type:eventKey} }, function (err, response) {
      if (err) {
        utils.flashMsg('show', 'Error while setting Poster Basic Profile');
        utils.logger('error', 'Set Poster Basic Profile Error -->', err);
      } else {
        if (response.data.Code == 200 && response.data.Status == true) {
          _this.setState({availability_type: eventKey});
          let userData = utils.getCurrentUser();
          if (userData) {
            userData.availability_type = eventKey;
  ...
  setSeekerData(userObj, listsObj) {
    ...
    const availability_type = userObj.availability_type;
    ...
    for (const [key, value] of Object.entries(constant.AVAILABILITY_TYPE)) {
      avails.push(<MenuItem eventKey={value['id']}>{value['label']}</MenuItem>);
    }
    this.setState({ avail_dropdown: avails });
  ...
  openConfirmDeleteModal() {
    ...
    _that.setState({
      modalPopupObj: {
        ...
        msg: constant['POPUP_MSG']['DELETE_ACCOUNT'],
        noBtnText: 'Cancel',
        yesBtnText: 'Confirm',
        textarea: true,
        ...
        yesBtnAction: function(reason) {
          let req = {};
          req.user_id = _that.state.userId;
          req.reason = reason
          utils.apiCall('DELETE_USER_ACCOUNT', { 'data': req }, function(err, response) {
            if (err) {
          ...
          <div className="section-head">
            ...
            {this.isOtherProfile() ? null : (
              <div className="d-inline pull-right">
                <button
                  type="button"
                  className="btn edit-profile-btn ml-2"
                ...
                <button 
                  ...
                  className="mr-2 btn btn-danger edit-profile-btn" 
                  ...
                  >
                    <div className="basic-info-preview">
                      ...
                      {!this.isOtherProfile() ?(
                        <div className="row">
                          <div className="pull-right">
                            <Dropdown
                              onSelect={this.handleAvailSelect}
                              className="dropdown-container"
                              id={`dropdown-basic-primary`}
                            >
                              <Dropdown.Toggle className="dropdown-order-toggle">
                                Set my availability to work
                              </Dropdown.Toggle>
                              <Dropdown.Menu>
                                {this.state.avail_dropdown}
                              </Dropdown.Menu>
                            </Dropdown>
                          </div>
                        </div>
                      ...
                      <div className="basic-info-content pull-left">
                        ...
                        {this.state.availability_type!=4?(
                          <address>
                            <i className="fa fa-info-circle" aria-hidden="true"></i>
                            {Object.entries(constant.AVAILABILITY_TYPE)[this.state.availability_type][1].label}
                          </address>
                        ):('')}
                        <address>
                    ...
                    <div className="job-type-preview">
                      ...
                      <div className="pull-left left-content">
                        ...
                        <div>
                          <h4 className="mt-30">
                            Desired Job Type and Compensation
                          </h4>
