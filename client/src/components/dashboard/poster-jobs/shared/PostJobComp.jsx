...
export default class PostJobComp extends React.Component {
  constructor(props) {
    ...
    this.state = {
      job: {
        ...
        total: '',
        remainingAmount: '',
        paymentDetails: [{ rate: '', delivery: '', dueDate: '' }],
        setting_id: '',
        currentRate: '',
        negotiateLaterStatus: false,
      },
      diableHours: false,
      formError: {},
    ...
    this.validateAndCalculateAmount = this.validateAndCalculateAmount.bind(
      this
    );
      ...
      if (err) {
        ...
        if (utils.isResSuccess(response)) {
          ...
          for (let jobTypeObj of data['job_types']) {
            jobTypes.push(
              <option key={jobTypeObj['_id']} value={jobTypeObj['label']}>
                {jobTypeObj['label']}
              </option>
            );
          }
          ...
          for (let paymentTypeObj of data['payment_types']) {
            paymentTypes.push(
              <option
                key={paymentTypeObj['_id']}
                value={paymentTypeObj['name']}
              >
                {paymentTypeObj['name']}
              </option>
            );
          ...
          for (let workLocationsObj of data['work_locations']) {
            workLocations.push(
              <option
                key={workLocationsObj['_id']}
                value={workLocationsObj['_id']}
              >
                {workLocationsObj['name']}
              </option>
            );
          }
          ...
          stateObj['hoursType'] = employmentTypes[0]['value'];
          stateObj['setting_id'] = workLocations[0]['key'];

          that.setState({
            job: stateObj,
            defaultJobType: data['job_types'][0]['name'],
            defaultPaymentType: data['payment_types'][0]['name'],
            job_type_dropdown: jobTypes,
            payment_type_dropdown: paymentTypes,
            practice_area_dropdown: practiceAreas,
            skill_dropdown: skills,
            state_dropdown: states,
            work_location_dropdown: workLocations,
  ...
  handleOnBlur(ev, key) {
    ...
    if (
      key == 'zipCode' &&
      this.state.work_location_dropdown[0] &&
      this.state.job.setting_id !== this.state.work_location_dropdown[0]['key']
    ) {
      if (val) {
        var text = /^[0-9]+$/;
        if (val.length == 5) {
    ...
    } else if (key === 'rate' && !this.state.job.negotiateLaterStatus) {
      if (!val) {
        formError[key] = constant['ENTER_RATE'];
      } else if (!Number(val)) {
        formError[key] = constant['RATE_ERROR'];
      }
    } else if (key === 'hours' && !this.state.job.negotiateLaterStatus) {
      if (!val) {
  ...
  calculateAmount() {
    ...
    setTimeout(function() {
      var subtotal =
        utils.ENUM.RATE_TYPE.FIXED == stateObj.rateType
          ? stateObj.rate || 0
          : (stateObj.rate || 0) * (stateObj.hours || 0);

      isNaN(subtotal) && (subtotal = 0);
      stateObj.subTotal = parseFloat(subtotal).toFixed(2);
      formError.subTotal =
        stateObj.subTotal < 100 ? constant['MIN_JOB_AMOUNT'] : false;
      var total =
        parseFloat(stateObj.subTotal) +
        parseFloat(
          parseFloat((stateObj.subTotal * stateObj.currentRate) / 100).toFixed(
            2
          )
        );
      ...
      _this.setState({
        legablyChargesForFixed: parseFloat(
          (subtotal * stateObj.currentRate) / 100
        ).toFixed(2)
      });
      if (utils.ENUM.RATE_TYPE.FIXED == stateObj.rateType) {
        stateObj.hours = 0;
  ...
  validateForm(job) {
    ...
    var nonValidateFieldsArr = [
      ...
      'city',
      'zipCode',
      'negotiateLaterStatus'
    ];
    if(job.negotiateLaterStatus){
      nonValidateFieldsArr = nonValidateFieldsArr.concat(['rate','hours','subTotal','remainingAmount','total']);
    }
    for (var key in job) {
      ...
      if (Array.isArray(job[key])) {
        ...
        if (key === 'zipCode') {
          if (
            this.state.work_location_dropdown[0] &&
            this.state.job.setting_id ==
              this.state.work_location_dropdown[0]['key']
          )
            continue;
          else {
            if (job[key]) {
          ...
          if (
            job.jobType == '1099' &&
            job.paymentType == 'Hourly Rate/Fixed Fee'
          ) {
            formError[key] = false;
            validForm = true;
          } else {
    ...
    if (
      !(
        job.jobType == '1099' &&
        (job.paymentType == 'Hourly Rate/Fixed Fee' || job.negotiateLaterStatus)
      )
    ) {
      ...
      if (err) {
        ...
        if (utils.isResSuccess(response)) {
          ...
          that.setState(
            {
              legablyChargesForFixed: parseFloat(
                (data.subTotal * job.currentRate) / 100
              ).toFixed(2),
              job: job,
              diableHours:
                data['rateType'] === utils.ENUM.RATE_TYPE.FIXED
                  ? 'disabled'
                  : false
            },
            function() {
  ...
  clearForm() {
    this.setState({
      job: {
        ...
        remainingAmount: '',
        paymentDetails: [{ rate: '', delivery: '', dueDate: '' }],
        setting_id: this.state.work_location_dropdown[0]['key'],
        negotiateLaterStatus: false
      },
      legablyChargesForFixed: 0
    });
  ...
  createJob(e, status, callback) {
    ...
    job.status = status;
    if (job.jobType == '1099') {
      job.paymentDetails = [{ rate: Number(0), delivery: '', dueDate: '' }];
    }
    if(job.negotiateLaterStatus){
      job.hours = 0;
      job.rate = 0;
    ...
    utils.apiCall('CREATE_JOB', { data: job }, function(err, response) {
      if (err) {
        ...
        if (response.data.Code == 200 && response.data.Status == true) {
          utils.apiCall('DECREASE_JOB_CNT', {}, function(err, response) {
            if (err) {} else {
              // console.log("DECREASE_JOB_CNT:", response.data)      
            }
          });      
          callback();
        } else {
          ...
          modalPopupObj: {
            ...
            msg:
              status === constant['STATUS']['INACTIVE']
                ? constant['POPUP_MSG']['JOB_SAVE_SUCCESS']
                : constant['POPUP_MSG']['JOB_POST_SUCCESS'],
  ...
  updateJob(evt, status) {
    let callback = () => {
      helper.openSuccessMessagePopup(this, 'JOB_UPDATE_SUCCESS', () => {
        utils.changeUrl(
          constant['ROUTES_PATH']['MY_POSTED_JOBS'] + '/' + this.props.jobId
        );
      });
    };
  ...
  validStartDate(current) {
    return moment(current.format(constant['DATE_FORMAT'])).isSameOrAfter(
  ...
  renderDay(props, currentDate, selectedDate) {
    return (
      <td {...utils.getUpdatedCalenderProps(props, currentDate)}>
        {currentDate.date()}{' '}
      </td>
    );
  }
        ...
        <div className="job-posting-card card">
          <div className="row">
            <div className="col-sm-8">
              ...
              >
                ...
                <select
                  ...
                  value={job.jobType}
                  onChange={e => this.changeInput(e, 'jobType')}
                  onBlur={e => this.handleOnBlur(e, 'jobType')}
                >
                  {this.state.job_type_dropdown}
                </select>
                {this.state.formError.jobType ? (
                  <p>
                    <span> Please select job type </span>
                  </p>
                ) : (
                  ''
                )}
            ...
            <div className="col-sm-4">
              <div
                className={
                  this.state.formError.paymentType
                    ? 'form-group global-error'
                    : 'form-group'
                }
              >
                ...
                <select
                  ...
                  onChange={e => this.changeInput(e, 'paymentType')}
                  onBlur={e => this.handleOnBlur(e, 'paymentType')}
                >
          ...
          <div
            className={
              this.state.formError.jobHeadline
                ? 'form-group global-error'
                : 'form-group'
            }
          >
            ...
            <input
              name="jobHeadline"
              className="form-control"
              placeholder="Unique job titles make searching easier and notifications more effective"
              type="text"
              maxLength="150"
              value={job.jobHeadline}
              onChange={e => this.changeInput(e, 'jobHeadline')}
              onBlur={e => this.handleOnBlur(e, 'jobHeadline')}
            />
            {this.state.formError.jobHeadline ? (
              <p>
                <span> Please enter job headline </span>
              </p>
            ) : (
              ''
            )}
          ...
          <div className="row">
            <div
              className={
                job.showOthers == 'true' || job.showOthers == true
                  ? 'col-sm-4'
                  : 'col-sm-6'
              }
            >
              <div
                className={
                  this.state.formError.practiceArea
                    ? 'form-group global-error'
                    : 'form-group'
                }
              >
                ...
                {this.state.formError.practiceArea ? (
                  <p>
                    <span> Please select practice area </span>
                  </p>
                ) : (
                  ''
                )}
            ...
            >
              <div
                className={
                  this.state.formError.skillsNeeded
                    ? 'form-group global-error'
                    : 'form-group'
                }
              >
                ...
                {this.state.formError.skillsNeeded ? (
                  <p>
                    <span> Please select skills needed </span>
                  </p>
                ) : (
                  ''
                )}
            ...
            <div className="col-sm-4">
              ...
              >
                ...
                <input
                  value={job.others}
                  onBlur={e => this.handleOnBlur(e, 'others')}
                  onChange={e => this.changeInput(e, 'others')}
                  name="others"
                  className="form-control"
                  placeholder="Other skills needed"
                  type="text"
                />
              </div>
            </div>
          ...
          <div
            className={
              this.state.formError.jobDescription
                ? 'form-group global-error'
                : 'form-group'
            }
          >
            ...
            <textarea
              ...
              onChange={e => this.changeInput(e, 'jobDescription')}
              onBlur={e => this.handleOnBlur(e, 'jobDescription')}
            ></textarea>
            {this.state.formError.jobDescription ? (
              <p>
                <span> Please enter job description </span>
              </p>
            ) : (
              ''
            )}
          ...
          <div className="row">
            <div className="col-sm-6">
              <div
                className={
                  this.state.formError.setting_id
                    ? 'form-group global-error'
                    : 'form-group'
                }
              >
                ...
                <select
                  name="setting_id"
                  className="form-control"
                  value={job.setting_id}
                  onChange={e => this.changeInput(e, 'setting_id')}
                  onBlur={e => this.handleOnBlur(e, 'setting_id')}
                >
            ...
            <div className="col-sm-6">
              ...
              >
                ...
                <select
                  ...
                  onBlur={e => this.handleOnBlur(e, 'state')}
                >
                ...
                <div className="col-sm-6">
                  ...
                  >
                    ...
                    <input
                      ...
                      type="text"
                      value={job.zipCode}
                      onChange={e => this.validateLength(e, 'zipCode', 99999)}
                    />
                    {this.state.formError.zipCode ? (
                      <p>
                        <span> {this.state.zipErr} </span>
                      </p>
                    ) : (
                      ''
                    )}
          ...
          <div className="row">
            <div className="col-sm-3">
              <div className="negotiate-later checkbox pmd-default-theme ">
                <label className="pmd-checkbox">
                  <input
                    ref="negotiate_later"
                    type="checkbox"
                    value={true}
                    checked={job.negotiateLaterStatus}
                    onChange={e => this.changeInput(e, 'negotiateLaterStatus')}
                    name="negotiate_later"
                  />
                  <span className="pmd-checkbox-label">&nbsp;</span>
                  <span style={{ textTransform: 'capitalize' }}>
                    Negotiate Specific Terms Later
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="posting-two-cols">
            <div className="form-group">
              ...
              <div
                className={
                  !job.negotiateLaterStatus && this.state.formError.rate
                    ? 'global-error inline-to-block'
                    : 'inline-to-block'
                }
              >
                {/* {!(job.negotiateLaterStatus) && ( */}
                <input
                  className="form-control amount-dollar-bg text-left custom-num"
                  placeholder="Rate"
                  type="text"
                  disabled={job.negotiateLaterStatus}
                  value={job.rate}
                  onChange={e =>
                    this.validateAndCalculateAmount(e, 'rate', 999999)
                  }
                  onBlur={e => this.handleOnBlur(e, 'rate')}
                />
                {/* )} */}

                {(this.state.formError.rate && !job.negotiateLaterStatus) ? (
                  <p>
                    <span> {this.state.formError.rate} </span>
                  </p>
                ) : (
                  ''
                )}
              ...
              <ul className="tabbed-radio-btns">
                <li>
                  <input
                    ...
                    name="h-f"
                    defaultChecked={
                      job.rateType == utils.ENUM.RATE_TYPE.HOURLY ? 1 : 0
                    }
                    defaultValue={utils.ENUM.RATE_TYPE.HOURLY}
                    onClick={e => this.onChangeHoursType(e, 'rateType')}
                  />
                  <span>Hourly</span>
                </li>
                <li>
                  <input
                    type="radio"
                    name="h-f"
                    defaultChecked={job.rateType == utils.ENUM.RATE_TYPE.FIXED ? 1 : 0}
                    defaultValue={utils.ENUM.RATE_TYPE.FIXED}
                    onClick={e => this.onChangeHoursType(e, 'rateType')}
                  />
                  <span>Fixed</span>
                </li>
          ...
          {!(
            ...
            <div className="posting-two-cols hours-col">
              <div className="form-group">
                ...
                <div
                  className={
                    this.state.formError.hours &&
                    !job.negotiateLaterStatus &&
                    job.rateType != utils.ENUM.RATE_TYPE.FIXED
                      ? 'global-error inline-to-block'
                      : 'inline-to-block'
                  }
                >
                  <input
                    className="form-control custom-num"
                    placeholder="00"
                    type="text"
                    value={job.hours}
                    onChange={e =>
                      this.validateAndCalculateAmount(e, 'hours', 999)
                    }
                    onBlur={e => this.handleOnBlur(e, 'hours')}
                    disabled={
                      this.state.diableHours || job.negotiateLaterStatus
                    }
                  />
                  {this.state.formError.hours ? (
                    <p>
                      <span> {this.state.formError.hours} </span>
                    </p>
                  ) : (
                    ''
                  )}
                ...
                <div className="pt-ft-w-m">
                  <ul className="tabbed-radio-btns">
                    {this.state.employment_type_dropdown.map(
                      (hoursType, index) => (
                        <li key={index}>
                          <input
                            type="radio"
                            name="pt-ft"
                            defaultChecked={hoursType.value == job.hoursType ? 1 : 0}
                            defaultValue={hoursType.value}
                            onClick={e => this.changeInput(e, 'hoursType')}
                            disabled={
                              this.state.diableHours || job.negotiateLaterStatus
                            }
                          />
          ...
          <div className="estimated-section">
            {!(
              ...
              <div className="estimated-amount-section">
                ...
                <p>
                  Based on the information you have provided, the estimated
                  amount payable to your selected candidate is indicated below.
                </p>
                <div className="row">
                  <div className="col-sm-4">
                    <div
                      className={
                        this.state.formError.subTotal &&
                        !job.negotiateLaterStatus
                          ? 'form-group global-error'
                          : 'form-group'
                      }
                    >
                      ...
                      <input
                        className="form-control amount-dollar-bg"
                        placeholder="Total"
                        type="text"
                        defaultValue={job.subTotal}
                        disabled={job.negotiateLaterStatus}
                      />
                      {this.state.formError.subTotal &&
                      !job.negotiateLaterStatus ? (
                        <p>
                          <span> {this.state.formError.subTotal} </span>
                        </p>
                      ) : (
                        ''
                      )}
            ...
            {!(
              ...
              <div className="estimated-schedule-section">
                ...
                <p>
                  The estimated payment and deliverable schedule allows you to
                  disperse the estimated amount payable to your selected
                  candidate over the course of your project or task based upon
                  the completion of specific deliverables. If there is only one
                  deliverable, simply allocate the estimated amount payable to
                  your selected candidate to that single item.{' '}
                </p>
                ...
                {job.paymentDetails.map((paymentDetail, index) => (
                  <ul className="payment-details row" key={index}>
                    <li
                      className={
                        paymentDetail.errorMessage && !job.negotiateLaterStatus
                    ...
                    >
                      ...
                      <input
                        className="amount-dollar-bg custom-num"
                        type="text"
                        placeholder="Payment"
                        value={paymentDetail.rate}
                        disabled={job.negotiateLaterStatus}
                        onChange={e => this.setPaymentDetails(e, 'rate', index)}
                      />
                      <p className="static">
                        <span>
                    ...
                    <li className="col-sm-4">
                      ...
                      <input
                        ...
                        value={paymentDetail.delivery}
                        disabled={job.negotiateLaterStatus}
                        onChange={e =>
                          this.setPaymentDetails(e, 'delivery', index)
                        }
                      />
                    </li>
                    <li className="col-sm-4">
                      ...
                      <Datetime
                        onChange={date => this.handleDueDate(date, index)}
                      ...
                      {paymentDetail.dueDate ? (
                        <span
                          className="clear"
                          onClick={e => this.handleDueDate('', index)}
                        ></span>
                      ) : null}
                    </li>
                    {job.paymentDetails.length > 0 && index != 0 ? (
                      <p
                        className="less"
                        onClick={e => this.removePaymentDetail(index)}
                      >
                    ...
                    {job.paymentDetails.length > 0 &&
                    index == job.paymentDetails.length - 1 &&
                    job.remainingAmount != 0 ? (
                      <p
                        className="add-more"
                        onClick={e => this.addPaymentDetail()}
                      >
                ...
                <ul className="remaining-detail">
                  <li>
                    ...
                    <input
                      ...
                      defaultValue={job.remainingAmount}
                      disabled={true}
                    />
                  </li>
                </ul>
            ...
            <div className="estimated-total-cost-section">
              {!(
                ...
                <div className="estimated-total-cost">
                  ...
                  <p>
                    The estimated total cost of this engagement is indicated
                    below. Please note that this amount is subject to change
                    pending final agreed upon terms negotiated between you and
                    the candidate. Once you have agreed to terms with your
                    candidate and are ready to move forward, the actual total
                    cost will be due and placed in escrow, payable to your
                    candidate per the final agreed upon payment and deliverable
                    schedule.
                  </p>
                  <p>
                    <span className="pull-left">
                      Estimated amount payable to selected candidate:
                    </span>
                    <span className="pull-right">
                      {isNaN(parseFloat(job.subTotal).toFixed(2))
                        ? '$00.00'
                        : '$' + job.subTotal}
                    </span>
                  ...
                  <p>
                    <span className="pull-left">
                      Estimated Legably service charge:
                    </span>
                    <span className="pull-right">
                      {isNaN(
                        parseFloat(this.state.legablyChargesForFixed).toFixed(2)
                      )
                        ? '$0.00'
                        : '$' + this.state.legablyChargesForFixed}
                    </span>
                    <span className="clearfix"></span>
                  </p>
                  <p className="clearfix grand-total">
                    ...
                    <span className="pull-right">
                      {isNaN(parseFloat(job.total).toFixed(2))
                        ? '$00.00'
                ...
                <p>
                  By posting this job you agree to pay an attorney for the work
                  outlined above based either on the hourly rate or the fixed
                  fee that the you both agree to on the Negotiated Terms screen.
                  You also agree to pay the Legably service charge at the same
                  time of payment to the attorney completing the work.
                </p>
              )}
            </div>
          </div>
        ...
        {this.props.isEditJobPage ? (
          <div className="nxt-prev-btns">
            <button
              ...
              onClick={e => this.updateJob(e, job.status)}
            >
              Update Job
            </button>
          ...
          <div className="nxt-prev-btns">
            <button
              name="postJob"
              className="d-block btn-primary btn pull-right ml-10"
              onClick={e => this.postOrSaveJob(e, constant['STATUS']['ACTIVE'])}
              disabled={!this.props.isEnablePostAJob}
            >
              Post Job
            </button>
            <button
              ...
              onClick={e =>
                this.postOrSaveJob(e, constant['STATUS']['INACTIVE'])
              }
              disabled={!this.props.isEnablePostAJob}
            >
              Save Job
