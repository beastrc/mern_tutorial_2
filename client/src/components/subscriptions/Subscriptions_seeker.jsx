...
export default class Subscription extends React.Component {
  ...
  getUserData() {
    ...
    utils.apiCall('GET_USER_DATA', {}, function (err, response) {
      if (err) {
        ...
        if(!user.job_seeker_info.is_profile_completed) {
          utils.changeUrl("/attorney-profile-get-started");
        }
      }
    })
  }
  ...
  getSubscribedPlan() {
    let _this = this;
    // get subscribed plan
    utils.apiCall('SUBSCRIBED_PLAN_SEEKER', {}, function (err, response) {
      if (err) {
      } else {
  ...
  getNewlySubscribedPlan() {
    let _this = this;
    utils.apiCall('GET_SUBSCRIBED_PLAN_SEEKER', {}, function (err, response) {
      if (err) {
        console.log(err)
      } else {
        ...
        if (userData) {
          ...
          userData.seeker_plan_id = response.data.plan['plan_id'];
  ...
  getAllPlans() {
    ...
    utils.apiCall('SUBSCRIBE_PLANS_SEEKER', {}, function (err, response) {
      if (err) {
        console.log(err)
      } else {        
        _this.setState({
    ...
    _this.setState({
      modalPopupObj: {
        ...
        yesBtnAction: function (msg) {
          ...
          utils.apiCall('UPDATE_SUBSCRIBED_PLAN_SEEKER', {data:data}, function (err, response) {
            if (err) {
            } else {
              let userData = utils.getCurrentUser();
              if (userData) {
    ...
    if(cancel_active){
      ...
      _this.setState({
        modalPopupObj: {
          ...
          yesBtnAction: function (msg) {
            utils.modalPopup(popupType, 'hide', _this);
            utils.apiCall('CANCEL_SUBSCRIPTION_SEEKER', {
              data: {
  ...
  cancelScheduledSubscription(id) {
    ...
    _this.setState({
      modalPopupObj: {
        ...
        yesBtnAction: function (msg) {
          utils.modalPopup(popupType, 'hide', _this);
          utils.apiCall('CANCEL_SCHEDULED_PLAN_SEEKER', {data: {subscription_id:id}}, function (err, response) {
            if (err) {
            } else {
