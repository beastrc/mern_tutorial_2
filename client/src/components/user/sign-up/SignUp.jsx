...
export default class SignUp extends React.Component {
  ...
  render() {
    return (
      ...
      //           <input name="code" type="text" className="form-control" id="code" placeholder="SignUp Code" value={this.state.code} onChange={this.handleUserInput} />
      //         </div>
      ...
      <div className="le_form-wrapper sign-up-page-wrapper">
        <h4 className="le-form-title le_f-bold le_text-center le_text-black">Sign Up</h4>
        <form className="le_sign-in-form sign-in-form" onSubmit={this._handleClick}>
          <div className="le_row">
          <div className={this.state.formErrors.first_name !== '' ? 'le_form-group form-group global-error le_col-md-6' : 'le_form-group remove-form-group le_col-md-6'}>
            <label className="le_color-lgray font-13 le_f-light" htmlFor="fname">first name*</label>
            <input name="first_name" type="text" className="le_form-control le_input-bg" id="fname" placeholder="First Name" value={this.state.first_name} onBlur={this.handleInputOnBlur} onChange={this.handleUserInput} />
            <p className="le_m-0"><span>{this.state.formErrors.first_name !== '' ? this.state.formErrors.first_name : ''}</span></p>
          </div>
          <div className={this.state.formErrors.last_name !== '' ? 'le_form-group form-group global-error le_col-md-6' : 'remove-form-group le_col-md-6'}>
            <label className="le_color-lgray font-13 le_f-light" htmlFor="lname">last name*</label>
            <input type="text" className="le_form-control le_input-bg" name="last_name" id="lname" placeholder="Last Name" value={this.state.last_name} onBlur={this.handleInputOnBlur} onChange={this.handleUserInput} />
            <p className="le_m-0"><span>{this.state.formErrors.last_name !== '' ? this.state.formErrors.last_name : ''}</span></p>
          </div>
          </div>
          <div className={this.state.formErrors.email !== '' ? 'form-group global-error' : 'remove-form-group'}>
            <label className="le_color-lgray font-13 le_f-light" htmlFor="email">Email* </label>
            <input type="text" className="le_form-control email-id le_input-bg" name="email"
              placeholder="Email"
              value={this.state.email}
              onBlur={this.handleInputOnBlur} onChange={this.handleUserInput} />
            <p><span>{this.state.formErrors.email !== '' ? this.state.formErrors.email : ''}</span></p>
          </div>
          <div className={this.state.formErrors.password !== '' ? 'form-group global-error' : 'le_form-group remove-form-group'}>
            ...
            <div className="pwd-wrapper le_position-relative">
              <input type="password" id="pwd" className="pswd le_form-control le_input-bg" name="password" placeholder="Password" value={this.state.password} onBlur={this.handleInputOnBlur} onChange={this.handleUserInput} />
          ...
          <div className={this.state.formErrors.confirm_password !== '' ? 'form-group global-error' : 'le_form-group remove-form-group'}>
            <label className="le_color-lgray font-13 le_f-light" htmlFor="repwd">Retype Password* </label>
            <div className="pwd-wrapper le_position-relative">
              <input type="password" className="pswd le_form-control le_input-bg" id="repwd" name="confirm_password" placeholder="Retype Password" value={this.state.confirm_password} onBlur={this.handleInputOnBlur} onChange={this.handleUserInput} />
              <span onClick={this.showPassword} className="le_eye-icon eye le_position-absolute le_cursor"><i className={this.state.showPass ? "fa fa-eye" : "fa fa-eye-slash"}></i></span>
            </div>
            <p><span>{this.state.formErrors.confirm_password !== '' ? this.state.formErrors.confirm_password : ''}</span></p>
          </div>
            <div className='form-group'>
              <label className="le_color-lgray font-13 le_f-light" htmlFor="code">Code</label>
              <input name="code" type="text" className="form-control le_form-control le_input-bg" id="code" placeholder="SignUp Code" value={this.state.code} onChange={this.handleUserInput} />
            </div>
          <div className={this.state.formErrors.recapcthaValidErr !== '' ? 'form-group checkbox global-error responive-captcha' : 'remove-form-group checkbox responive-captcha'} >
            {this.state.sitekey ? <Recaptcha ref={e => recaptchaInstance = e} sitekey={this.state.sitekey} render="explicit" verifyCallback={this.verifyCallback} onloadCallback={this.resetCallback} /> : ''}
            <p><span>{this.state.formErrors.recapcthaValidErr !== '' ? this.state.formErrors.recapcthaValidErr : ''}</span></p>
          </div>
          <div className={this.state.formErrors.termCond !== '' ? 'form-group checkbox global-error' : 'remove-form-group checkbox'}>
            <label className="pmd-checkbox le_check-rememeberMe le_check-rem le_color-lgray font-13 le_f-light">
              <input name="termCond" type="checkbox" checked={this.state.termCond} onChange={this.handleInputChange} /><span className="pmd-checkbox-label le_input-rememeberMe font-12 le_f-light">&nbsp; By clicking <q>Sign Up</q> I agree to Legably's Terms of Service
              </span></label>
            <p><span>{this.state.formErrors.termCond !== '' ? this.state.formErrors.termCond : ''}</span></p>
          </div>
          <div className="btns">
            <button type="submit" className="le_btn btn le_f-bold sign-in-btn sign-up-btn">Sign up</button>
          </div>
        ...
        <div className="already-signed font-13 le_f-light">Already have an account? <Link to={constant['ROUTES_PATH']['SIGN_IN']}>Sign In</Link></div>
        <ModalPopup modalPopupObj={this.state.modalPopupObj} />
      </div>
    );
  }
