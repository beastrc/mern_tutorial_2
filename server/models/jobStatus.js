const mongoose = require('mongoose'), post_jobs = mongoose.model('post_jobs');
var rfr = require('rfr'), async = require('async');
...
function _updateJobStatusAfterAllChecks(updateDataObj, result, cb, fromCancelOrder = false) {
  ...
  if (updateDataObj['dbQueryParams']['status'] === (constant['JOB_STEPS']['S_PENDING'] * -1) && !fromCancelOrder) {
    ...
    let resObj = Object.assign({}, utils.getErrorResObj());
    jobStatusSchema.updateQuery(updateDataObj['dbQueryParams'], async function (uErr, uRes) {
      if (uErr) {
        utils.callCB(cb, resObj);
        ...
        if(user.job_seeker_info.is_premium > 0) {
          user.job_seeker_info.is_premium = user.job_seeker_info.is_premium - 1;
          await userSchema.updateOne({email: email}, {job_seeker_info: user.job_seeker_info}, function(req, res){
...
function getJobSeekerDetailByJobId(stepObj, cb) {
  utils.writeInsideFunctionLog('jobStatus', 'getJobSeekerDetailByJobId', stepObj);

  var dbQueryParams =  { 'job_id':  stepObj.jobId };
 

  jobStatusSchema.findQuery(dbQueryParams, function (cErr, cResult) {
    if (cErr) {
      utils.writeErrorLog('jobStatus', 'getJobSeekerDetailByJobId', 'Error while getting details for specific job id', cErr, dbQueryParams);
      cb(cErr, {});
    } else {

      let seekerEmail = {
        '_id': cResult[0].user_id
      }

      var query = userSchema.find(seekerEmail).select({
        "email": 1,
        "_id": 1,
        "first_name": 1,
        "last_name":1
      ...
      query.exec(function (uErr, uRes) {
        if (uErr) {
          utils.writeErrorLog('jobStatus', 'getJobSeekerDetailByJobId', 'Error while getting uErr for given job id', cErr, seekerEmail);
          cb(uErr, {});
        } else {

         post_jobs.getPostjobData(dbQueryParams,function (jbErr, jbRes) {

            if (jbErr) {
              utils.writeErrorLog('jobStatus', 'getJobSeekerDetailByJobId', 'Error while getting jbErr for given job id', cErr, seekerEmail);
              cb(jbErr, {});
            } else {
              let data = {
                'Job_id': stepObj.jobId,
                'seeker_user_data': uRes[0],
                'JobDetails': jbRes[0]
              }
              cb(null, data);
            }
