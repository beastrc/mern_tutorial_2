...
function _sendMailOnPostJob(userDataObj, jobDataObj) {  
  ...
  post_jobs.getCriteriaValues({ job_id: jobDataObj._id}, function(pErr, pRes){
    if(pErr){
      utils.callCB(cb, resObj);
    }else{
      pRes[0].user_id = userDataObj._id;
      users.getCandidatesByPostedJob(pRes[0],function(ppErr, ppRes){
        if(ppErr){
          utils.callCB(cb, resObj);
        }else{
          ppRes.filter(function(item){
            let mailObj = {
              ...
              url: config['server']['end_point']+"/project-search/"+jobDataObj._id,
            };
    ...
    post_jobs.saveData(req['body'], function(err, resp) {
      if (err) {
        ...
        if (req.body.status === constant['STATUS']['ACTIVE']) {
          _sendMailOnPostJob(userData, resp);
        }
        callback({
          Code: 200,
          Status: true,
  ...
  if (stepDataObj['userRole'] === constant['ROLE']['SEEKER']) {
    switch (stepDataObj['step']) {
      ...
      case 105:
        ...
        break;
      default:
        // mail to send from here 
        _sendReviewEmail(stepDataObj,callback)
    }
  } else {
    let absStep =
      stepDataObj['step'] < 0
...
function _sendReviewEmail(stepDataObj,callback){
  jobStatusModel.getJobSeekerDetailByJobId(stepDataObj, function(jbsErr, jbsRes){

  /* main functionality sated from here */

  //mail obj for template
  let mailObj = {
    jobId: stepDataObj.jobId,
    userName: `${stepDataObj.userData.first_name} ${stepDataObj.userData.last_name}`,
    email: jbsRes['seeker_user_data'].email,
    jobHeadline: jbsRes['JobDetails'].jobHeadline ,
    jobPostDate: moment(jbsRes['JobDetails'].created_at).format('MMMM Do YYYY')
  };

  //poster email id to be configured here
  config.mailOptions.to = stepDataObj.userData.email;
  config.mailOptions.subject = `Review Your Project on Legably ?`;
  config.mailOptions.html = mailHelper.getMailTmpl( 'REVIEW_EMAIL', mailObj);

  utils.writeInsideFunctionLog('postJobs', '_sendReviewEmail', mailObj);
  mailHelper.sendMailInBackground(stepDataObj.userData.email, config.mailOptions.subject , 'REVIEW_EMAIL', mailObj);
  
  // Job seeker email
  mailObj.userName =  `${jbsRes['seeker_user_data'].first_name} ${jbsRes['seeker_user_data'].last_name}`
  mailObj.email = stepDataObj.userData.email;
  mailHelper.sendMailInBackground(jbsRes['seeker_user_data'].email, config.mailOptions.subject, 'REVIEW_EMAIL', mailObj);
    ...
    helper.checkUserLoggedIn(req.headers.token, function(err, result) {
      if (err) {
        ...
        if (validateObj.isValid) {
          if(req.body.negotiateLaterStatus){
          ...
          if (validateObj.isValid) {
            if (
              (req.body.jobType == '1099' &&
              req.body.paymentType == 'Hourly Rate/Fixed Fee') || req.body.negotiateLaterStatus
            ) {
    ...
    helper.checkUserLoggedIn(req.headers.token, function(err, result) {
      if (err) {
        ...
        let dbQueryParams = {
          ...
          selectedOrderCate: reqBody.selectedOrderCate,
          searchKeywords: reqBody.searchKeywords
        };
        let stateObj =
        ...
        //console.log(queryObj.query['$and'][4]);
...
function updateStatus(req, res, cb) {
  ...
  helper.checkUserLoggedIn(req['headers']['token'], async function(err, result) {
    if (err) {
      ...
      let headline = req['body']['jobHeadline'];
      ...
      if(req['body']['status'] == -2 || req['body']['status'] == -1) {
        try {
          ...
          job_status.findUserDetail(dataObj, async function(jErr, jRes) {
            if(jErr){
              callback(constant['OOPS_ERROR']);
              utils.writeErrorLog(
                'postJobs',
                'jobStatus::findUserDetail',
                'Error while finding out specified job_id',
                nErr,
                queryObj
              );
            }else if(jRes && jRes.length>0) {
              for (let jobStatus of jRes) {
                let mailObj = {
                  headline: headline
                };
                mailHelper.sendMailInBackground(jobStatus.userDetail.email, 'Project is deleted', 'DELETE_PROJECT_SUCCEED_USER', mailObj);
              }
              await job_status.deleteMany({job_id: req['body']['job_id']});
            }
          });
          ...
          mailHelper.sendMailInBackground(constant['MAIL_OBJ']['INFO_ID'], 'Successful Project Auto-Delete Attempt', 'DELETE_PROJECT_SUCCEED_LEGABLY', mailObj);
        } catch(err) {
          mailHelper.sendMailInBackground(result.email, 'Project delete failed', 'DELETE_PROJECT_FAILED', mailObj);
        }
      }
