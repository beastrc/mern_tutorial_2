...
function bookDemo(req, res, callback) {
  utils.writeInsideFunctionLog('universal', 'bookDemo');
  ...
  if (
    ...
    if (
      validator.alphaWithDashOnly(reqBody.userName) &&
      validator.maxLength(reqBody.userName, 50, true) &&
      validator.alphaWithDashOnly(reqBody.firmName) &&
      validator.maxLength(reqBody.firmName, 100, true) &&
      validator.emailValidation(reqBody.email)
    ) {
      // reqBody.message.trim()
      let mailObj = {
        userName: reqBody.userName.trim(),
        message: 'The above user has asked for the Legably demo.',
        email: reqBody.email.trim(),
        firmName : reqBody.firmName.trim()
      };

      //constant['MAIL_OBJ']['SUPPORT_ID']
      config.mailOptions.to = constant['MAIL_OBJ']['SUPPORT_ID'];
      config.mailOptions.subject = `${mailObj['userName']} | Book A Demo | Legably Support Center`;
      config.mailOptions.html = mailHelper.getMailTmpl(
        'BOOK_A_DEMO_SUPPORT',
        mailObj
      );
      ...
      config.transporter.sendMail(config.mailOptions, (error, info) => {
        if (error) {
          ...
          config.mailOptions.to = mailObj['email']; // For dynamic list of recievers
          config.mailOptions.subject =
            "We received your demo request! Here's what to expect next...";
          config.mailOptions.html = mailHelper.getMailTmpl(
            'BOOK_A_DEMO_USER',
            mailObj
          );
