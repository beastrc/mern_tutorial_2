    ...
    users.findOneQuery(dbQueryParams, function(err, result) {
      if (err) {
        ...
        if (utils.isObjectNotEmpty(result)) {
          users.comparePassword(password, result['password'], async function(uErr, uRes) {
            if (uErr) {
              utils.callCB(cb, resObj);
              ...
              if (uRes) {
                ...
                let subs_res_poster = await subscriptions.findOne({ email: result.email, status: { $in: ['active', 'trialing'] } });
                let subs_res_seeker = await subscription_seekers.findOne({ email: result.email, status: { $in: ['active', 'trialing'] } });
                if(subs_res_poster){
                  userData.subscription_status = 1;
                  userData.poster_plan_id = subs_res_poster.plan_id;
                }else{
                  userData.subscription_status = 0;
                }
                // if(subs_res_seeker){
                //   userData.subscription_status = 1;
                //   userData.seeker_plan_id = subs_res_seeker.plan_id;
                // }else{
                //   userData.subscription_status = 0;
                // }
