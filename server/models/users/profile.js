...
var helper = rfr('/server/models/shared/helper'),
  subscriptionModel = rfr('/server/models/subscriptions'),
...
function _updateUserProfile(
  ...
  helper.checkUserLoggedIn(token, function(err, result) {
    if (err) {
      ...
      if (
        result[userType].last_visited_page != 5 &&
        currentStep > result[userType].last_visited_page
      ) {
        req.body[userType].last_visited_page = currentStep;
      } else {
        req.body[userType].last_visited_page =
          result[userType].last_visited_page;
      }
      users.findProfile(
        result._id,
        userType,
        from,
        req.body[userType][from],
        req.body[userType].last_visited_page,
        function(fErr, fRes) {
          if (fErr) {
            callback({
            ...
            users.updateProfile(result._id, fRes, function(uErr, uRes) {
              if (uErr) {
                ...
                utils.writeErrorLog(
                  'profile',
                  '_updateUserProfile',
                  'Error while updating user detail',
                  uErr,
                  { _id: result._id }
                );
              } else {
                callback({
                  Code: 200,
                  Status: true,
                  Message: constant['SUCCESS_UPDATE_PROFILE']
                });
              }
            });
          }
...
function _deleteFile(directory) {
  utils.writeInsideFunctionLog('profile', '_deleteFile', {
    directory: directory
  });
  ...
  s3Bucket.deleteObject(params, function(err, data) {
    if (data) {
      ...
      utils.writeErrorLog(
        'profile',
        '_deleteFile',
        'Error while deleting file from s3 Bucket',
        err,
        params
      );
    }
  ...
  s3Bucket.deleteObjects(params, function(err, data) {
    if (data) {
      ...
      utils.writeErrorLog(
        'profile',
        'deleteFiles',
        'Error while deleting files from s3 Bucket',
        err,
        params
      );
    }
  ...
  s3Bucket.upload(uploadObj, function(err, data) {
    if (err) {
      ...
      utils.writeErrorLog(
        'profile',
        '_uploadFileOnAWS',
        'Error while uploading file on s3 Bucket',
        err
      );
    } else {
      cb(null, data);
  ...
  async.each(
    ...
    function(sample, callback) {
      ...
      var file =
        ...
        userId +
        '/' +
        sample['name']; // DESTINATION WHERE WE WANT TO UPLOAD FILE
      var validFormats = [
        'pdf',
    ...
    function(err, file) {
      if (err) {
        ...
        utils.writeErrorLog(
          'profile',
          '_uploadSamples',
          'Error while uploading writing samples',
          err
        );
      } else {
...
function _uploadFile(from, fileObj, userId, callback) {
  ...
  if (!!fileObj) {
    ...
    if (validateFileObj['isValidFile']) {
      ...
      _uploadFileOnAWS(
        fileName,
        validateFileObj['file']['encodedData'],
        function(err, data) {
          if (err) {
            callback(err);
          } else {
                ...
                if (error) {
                  ...
                  if (result) {
                    ...
                    callback({
                      Code: 400,
                      Status: false,
                      Message: constant['NO_RESOURCE_FOUND']
                    });
                  }
                }
              });
...
function basicProfile(req, res, callback) {
  ...
  if (
    ...
    !!req.body.first_name &&
    !!req.body.last_name &&
    !!req.body.job_seeker_info.basic_profile.basic_info.street_address &&
    !!req.body.job_seeker_info.basic_profile.basic_info.city &&
    !!req.body.job_seeker_info.basic_profile.basic_info.state_id &&
    !!req.body.job_seeker_info.basic_profile.basic_info.zipcode &&
    !!req.body.job_seeker_info.basic_profile.basic_info.phone_number &&
    !!req.body.job_seeker_info.basic_profile.practice_area_id &&
    req.body.job_seeker_info.basic_profile.do_you_have_malpractice_insurance
  ) {
    var eduCount = 0;
    var barCount = 0;
    if (!!req.body.job_seeker_info.basic_profile.education) {
      for (
        var i = 0;
        i < req.body.job_seeker_info.basic_profile.education.length;
        i++
      ) {
        if (
          !!req.body.job_seeker_info.basic_profile.education[i].school &&
          !!req.body.job_seeker_info.basic_profile.education[i].degree_id &&
          !!req.body.job_seeker_info.basic_profile.education[i].year
        ) {
          if (
            validator.maxLength(req.body.first_name, 50, true) &&
            validator.maxLength(req.body.last_name, 50, true) &&
            validator.maxLength(
              req.body.job_seeker_info.basic_profile.basic_info.street_address,
              250,
              true
            ...
            validator.maxLength(
              req.body.job_seeker_info.basic_profile.education[i]
    ...
    if (!!req.body.job_seeker_info.basic_profile.bar_admission) {
      for (
        var i = 0;
        i < req.body.job_seeker_info.basic_profile.bar_admission.length;
        i++
      ) {
        if (
          !!req.body.job_seeker_info.basic_profile.bar_admission[i]
            .bar_registration_number
          //&& validator.maxLength(req.body.job_seeker_info.basic_profile.bar_admission[i].bar_registration_number, 15, true)
        ) {
          barCount++;
        } else {
          callback({
    ...
    if (
      eduCount == req.body.job_seeker_info.basic_profile.education.length &&
      barCount == req.body.job_seeker_info.basic_profile.bar_admission.length
    ) {
      if (
        validator.alphaWithDashOnly(req.body.first_name) &&
        validator.alphaWithDashOnly(req.body.last_name)
      ) {
        if (
          validator.maxLength(
            req.body.job_seeker_info.basic_profile.basic_info.zipcode,
...
function experienceProfile(req, res, callback) {
  ...
  if (!!req.headers.token && !!req.body.job_seeker_info.experience) {
    if (req.body.job_seeker_info.experience.length == 0) {
      ...
      req.body.job_seeker_info.experience = _.sortBy(
        req.body.job_seeker_info.experience,
        function(o) {
      ...
      for (var i = 0; i < req.body.job_seeker_info.experience.length; i++) {
        if (
          !!req.body.job_seeker_info.experience[i].company_name ||
          !!req.body.job_seeker_info.experience[i].designation ||
          req.body.job_seeker_info.experience[i].employment_type_id.length >
            0 ||
          req.body.job_seeker_info.experience[i].skill_used_id.length > 0 ||
          !!req.body.job_seeker_info.experience[i]
          ...
          if (
            ...
            if (
              !!req.body.job_seeker_info.experience[i].start_date &&
              !!req.body.job_seeker_info.experience[i].end_date
            ) {
  ...
  if (!!req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token, function(loginErr, result) {
      if (loginErr) {
        ...
        var change_job_seeker_info = _removeBucketUrlFromSavedObj(
          req.body.job_seeker_info
        );
        req.body.job_seeker_info = change_job_seeker_info;

        if (
          validator.maxLength(
            req.body.job_seeker_info.network.lawyer_headline,
            150,
            false
          ) &&
          validator.maxLength(
            req.body.job_seeker_info.network.about_lawyer,
            700,
            false
          )
        ) {
          if (
            validator.linkedinLinkValidation(
              req.body.job_seeker_info.network.linkedin_link,
              false
            )
          ) {
            ...
            if (
              req.body.job_seeker_info.network.photo.hasOwnProperty('dataUrl')
            ) {
              _uploadFile(
                'photo',
                ...
                function(error, success) {
                  if (error) {
                    ...
                    var path = success.replace(
                      config.fileUploadPath,
                      config.hostPath + '/uploaded_files'
                    );
                    req.body.job_seeker_info.network.photo = path;
                    if (
                      req.body.job_seeker_info.network.resume.hasOwnProperty(
                        'dataUrl'
                      )
                    ) {
                      _uploadFile(
                        ...
                        function(err, succ) {
                          if (err) {
                            ...
                            if (
                              req.body.job_seeker_info.network.writing_samples
                                .length > 0
                            ) {
                              _uploadSamples(
                                req.body.job_seeker_info.network
                                  .writing_samples,
                                userId,
                                function(Error, Success) {
                                  if (Error) {
                                    ...
                                    req.body.job_seeker_info.network.writing_samples = req.body.job_seeker_info.network.writing_samples.concat(
                                      req.body.job_seeker_info.network
                                        .alreadyAddedSample
                                    );
                                    _updateUserProfile(
                      ...
                      if (
                        ...
                        req.body.job_seeker_info.network.writing_samples = req.body.job_seeker_info.network.writing_samples.concat(
                          req.body.job_seeker_info.network.alreadyAddedSample
                        );
                        _updateUserProfile(
                          req.headers.token,
                          3,
              ...
              if (
                ...
                if (!req.body.job_seeker_info.network.resumeUpdate) {
                  ...
                  if (req.body.job_seeker_info.network.deleteResumeLink) {
                    _deleteFile(
                      req.body.job_seeker_info.network.deleteResumeLink
                    );
                  }
                  req.body.job_seeker_info.network.resume = '';
                }
                if (
                  req.body.job_seeker_info.network.writing_samples.length > 0
                ) {
                  _uploadSamples(
                    req.body.job_seeker_info.network.writing_samples,
                    userId,
            ...
            callback({
              Code: 400,
              Status: false,
              Message: constant['INVALID_LINKEDIN_LINK']
            });
          }
        } else {
          callback({
...
function jobProfile(req, res, callback) {
  ...
  if (!!req.headers.token) {
    if (
      (req.body.job_seeker_info.job_profile.willing_to_work_locally == 'Y' &&
        req.body.job_seeker_info.job_profile.willing_to_work_location_id
          .length > 0) ||
      req.body.job_seeker_info.job_profile.willing_to_work_locally == 'N'
    ) {
      if (!!req.body.job_seeker_info.job_profile.desired_job_type) {
        async.each(
          req.body.job_seeker_info.job_profile.desired_job_type,
          function(desired_job_type, callback) {
            employment_types.findJobType(
              desired_job_type.employment_type_id,
              function(err, result) {
                if (err) {
                  ...
                  utils.writeErrorLog(
                    'profile',
                    'jobProfile',
                    'Error while getting employment type',
                    err,
                    { _id: desired_job_type.employment_type_id }
                  );
                } else {
                  if (result) {
                    if (result.name == 'Permanent') {
                      desired_job_type.min_amount = parseInt(
                        desired_job_type.min_amount
                      );
                      desired_job_type.max_amount = parseInt(
                        desired_job_type.max_amount
                      );
                      if (
                        desired_job_type.min_amount >= 50000 &&
                        desired_job_type.min_amount <= 200000 &&
                        desired_job_type.max_amount >=
                          desired_job_type.min_amount &&
                        desired_job_type.max_amount <= 200000
                      ) {
                        callback();
                      } else {
                      ...
                      if (
                        desired_job_type.min_amount >= 0 &&
                        desired_job_type.min_amount <= 2000 &&
                        desired_job_type.max_amount >=
                          desired_job_type.min_amount &&
                        desired_job_type.max_amount <= 2000
                      ) {
                        callback();
                      } else {
                        callback(constant['INVALID_AMT']);
...
function posterAvailProfile(req, res, callback) {
  utils.writeInsideFunctionLog('profile', 'posterAvailProfile');
  if (!!req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token, function(err, result) {
      if (err) {
        callback({ Code: 401, Status: false, Message: err });
      } else {
        users.updateProfile(result._id, req.body, function(uErr, uRes) {
          if (uErr) {
            callback({
              Code: 400,
              Status: false,
...
function posterBasicProfile(req, res, callback) {
  ...
  if (
    ...
    !!req.body.first_name &&
    !!req.body.last_name &&
    !!req.body.job_posters_info.basic_profile.basic_info.street_address &&
    !!req.body.job_posters_info.basic_profile.basic_info.city &&
    !!req.body.job_posters_info.basic_profile.basic_info.state_id &&
    !!req.body.job_posters_info.basic_profile.basic_info.zipcode &&
    !!req.body.job_posters_info.basic_profile.basic_info.phone_number
  ) {
    if (
      validator.alphaWithDashOnly(req.body.first_name) &&
      validator.alphaWithDashOnly(req.body.last_name) &&
      ...
      validator.maxLength(
        req.body.job_posters_info.basic_profile.firm_name,
        100,
        false
      ) &&
      validator.maxLength(
        req.body.job_posters_info.basic_profile.title,
        100,
        false
      )
    ) {
      if (
        (!!req.body.job_posters_info.basic_profile.website_url &&
          validator.websiteValidation(
            req.body.job_posters_info.basic_profile.website_url
          )) ||
        req.body.job_posters_info.basic_profile.website_url == ''
      ) {
...
function uploadDeliverable(fileObj, userId, jobId, key, callback) {
  utils.writeInsideFunctionLog('profile', 'uploadDeliverable', {
    userId: userId,
    jobId: jobId,
    key: key
  });
...
function downloadFile(filepath, callback) {
  utils.writeInsideFunctionLog('profile', 'downloadFile', {
    filepath: filepath
  });
  ...
  s3Bucket.getObject(params, function(err, data) {
    if (data) {
      ...
      utils.writeErrorLog(
        'profile',
        'downloadFile',
        'Error while downloading file from s3 Bucket',
        err,
        params
      );
    }
...
function deleteAccount(req, res, cb) {
  let resObj = Object.assign({}, req.body);
  if (!!req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token, async function(
      loginErr,
      result
    ) {
      if (loginErr) {
        ...
        try{
          ...
          // Has user posted any jobs?
          await post_jobs.find(query, async function(err, res) {
            const posted_jobs = res;

            for (var i = 0; i < posted_jobs.length; i++) {
              // Did anyone apply for this job?
              await job_status.find(
                {
                  job_id: posted_jobs[i].id,
                  status: {
                    $in: [
                      constant['JOB_STEPS']['S_PENDING'],
                      constant['JOB_STEPS']['IN_PROGRESS']
                    ]
                  }
                },
                async function(err, res1) {
                  if (res1.length === 0) {
                    // Delete post_job record
                    await post_jobs.deleteJob(posted_jobs[i].id, function(
                      err,
                      res
                    ) {});
                    await job_status.deleteOne({ job_id: posted_jobs[i].id });
                  } else {
                    // Was anyone hired for this job?
                    unable_msg = unable_msg + "You have posted jobs that are in progress. "
                    able_delete = false;
                  }
                }
              );
          ...
          await job_status.find(query1, async function(err, res3) {
            if (res3.length === 0) {
              // Delete all job status for this job
              await job_status.deleteMany({ user_id: resObj['user_id'] });
            } else {
              unable_msg = unable_msg + "You have applied jobs that are in progress. "
              able_delete = false;
            }
          });
          if(! await subscriptionModel.cancelSubscriptionByDeleteAccount(resObj['user_id'])){
            unable_msg = unable_msg + "There are errors in subscriptions. "
            able_delete = false;
          }

          users.findOne({ _id: resObj['user_id'] }, function(err, userData) {
            let mailObj = {
              firstName: userData['first_name'],
              lastName: userData['last_name'],
              email: userData['email'],
              deletedReason: resObj['reason']
            };
            
            if (!able_delete) {
              mailHelper.sendMailInBackground(
                constant['MAIL_OBJ']['INFO_ID'],
                'Unsuccessful User Auto-Delete Attempt',
                'DELETE_ACCOUNT_FAILED',
                mailObj
              );
              resObj['message'] = constant['DELETE_ACCOUNT_FAILED'] + unable_msg;
              resObj['code'] = constant['RES_OBJ']['CODE']['FAIL'];
              ...
              mailHelper.sendMailInBackground(
                constant['MAIL_OBJ']['INFO_ID'],
                'Successful User Auto-Delete Attempt',
                'DELETE_ACCOUNT_SUCCEED_LEGABLY',
                mailObj
              );
              mailHelper.sendMailInBackground(
                mailObj['email'],
                'Legably User Account Deleted',
                'DELETE_ACCOUNT_SUCCEED_USER',
                mailObj
              );

              users.deleteUser(resObj['user_id'], resObj['reason'], function(
                err,
                user
              ) {
                if (err) {
...
function getUserData(req, res, cb) {
  if (req.headers.token) {
...
function getMyNetwork(req, res, cb) {
  if (req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token, async function (err, result) {
      if (err) {
        cb({ Code: 401, Status: false, Message: err });
...
function removeUserInNetwork(req, res, cb) {
  if (req.headers.token) {
    helper.checkUserLoggedIn(req.headers.token, async function (err, result) {
      if (err) {
        cb({ Code: 401, Status: false, Message: err });
        ...
        const email = result.email;
        const remove_email = req.body['remove_email'];
        const user = await users.findOne({email: email});
        const remove_index = user.my_network.findIndex(n_email => n_email == remove_email);
        user.my_network.splice(remove_index, 1);
        await users.findOneAndUpdateQuery({query: { email: email }, data: {
          my_network: user.my_network
        }}, function(uErr, uRes) {
          if(uErr) {
            res.json({
              success: false,
