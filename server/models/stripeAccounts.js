...
//var webhook_baseurl = config['hostPath'];
var webhook_baseurl = 'http://a98185341a24.ngrok.io';

stripe.webhookEndpoints.list({
  limit: 100,
}).then((res) => {
  if(res.data.length){  //update webhook.
    stripe.webhookEndpoints.update(
      res.data[0].id,
      {
        url: webhook_baseurl+"/webhook",
        description: "stripe webhook",
        enabled_events: constant['WEBHOOK_ENABLE_EVENTS'],
      }
    ).then((res) => {
      console.log("updated webhook");
    });
  }else{ // create webhook if not exist.
...
function webhook(req, res) {
  ...
  switch(reqBody['type']) {
    ...
    case "customer.subscription.updated":
      subscriptionModel.subscriptionWebhook(reqBody);
      break;
  }
