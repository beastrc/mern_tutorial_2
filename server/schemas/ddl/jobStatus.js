...
jobStatus.statics.updateQuery = function(data, callback) {
  var _that = this;
  _that
    .find({ user_id: data.user_id, job_id: data.job_id })
...
jobStatus.statics.fetchAll = function(data, callback) {
  this.aggregate([
    ...
    {
      $addFields: {
        'job_details.stateId': { $toObjectId: '$job_details.state' }
      }
    },
    {
      $lookup: {
        from: 'states',
        localField: 'job_details.stateId',
        foreignField: '_id',
        as: 'job_details.state_info'
      }
    },
    { $unwind: '$job_details.state_info' },
...
jobStatus.statics.findUserDetail = function(data, callback) {
  this.aggregate([
    {
      $match: {
        job_id: mongoose.Types.ObjectId(data.job_id),
