...
var postJobs = new Schema({
  ...
  negotiateLaterStatus: { type: Boolean, default: false },
...
postJobs.index({ jobHeadline: 'text', jobDescription: 'text' });
...
postJobs.statics.deleteJob = function (id, callback) {
...
postJobs.statics.getAllJobs = function(data, callback) {
  ...
  var areaObj =
    data.practiceAreas && data.practiceAreas.length
      ...
      : {};
  var searchField = data.selectedOrderCate;
  var sortObj = {};
  sortObj[searchField] = data.selectedOrder;
  sortObj['_id'] = -1;
  ...
  this.aggregate([
    ...
    { $unwind: '$state_info' },
    //{ $sort: { searchField: Number(data.selectedOrder), _id: -1 } },
    { $sort: sortObj },
    { $limit: data.skip + data.limit },
    { $skip: data.skip },
    {
...
postJobs.statics.getCriteriaValues = function(data, callback) {
  this.aggregate([
    { 
      $match: { _id: mongoose.Types.ObjectId(data.job_id) } 
    },
